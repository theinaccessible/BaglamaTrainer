...
public class DataGenerator {
     ...
     * @return list of object
     */
    public static List<SazCategory> getSazCategory(Context ctx) {
        List<SazCategory> items = new ArrayList<>();
        TypedArray drw_arr = ctx.getResources().obtainTypedArray(R.array.saz_category_icon);
        TypedArray drw_arr_bg = ctx.getResources().obtainTypedArray(R.array.saz_category_bg);
        String[] title_arr = ctx.getResources().getStringArray(R.array.saz_category_title);
        String[] brief_arr = ctx.getResources().getStringArray(R.array.saz_category_brief);
        for (int i = 0; i < drw_arr.length(); i++) {
            SazCategory obj = new SazCategory();
            obj.image = drw_arr.getResourceId(i, -1);
